...
fn parse_raw<'a, T: fasteval::Num>(s: &str, slab: &'a mut Slab<T>) -> Result<ExpressionI, Error> {
    Parser::new().parse(s, &mut slab.ps)
}
fn ok_parse<'a, T: fasteval::Num>(s: &str, slab: &'a mut Slab<T>) -> ExpressionI {
...
fn aaa_test_b0() {
    ...
    ok_parse::<f64>("3.14 + 4.99999999999999", &mut slab);
    assert_eq!(format!("{:?}",&slab),
...
Err(Error::ParseFloat("4.999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999".to_string())));
    ok_parse::<f64>("3.14 + 0.9999", &mut slab);
    assert_eq!(format!("{:?}",&slab),
"Slab{ exprs:{ 0:Expression { first: EConstant(3.14), pairs: [ExprPair(EAdd, EConstant(0.9999))] } }, vals:{}, instrs:{} }");
    ok_parse::<f64>("3.14 + .9999", &mut slab);
    assert_eq!(format!("{:?}",&slab),
"Slab{ exprs:{ 0:Expression { first: EConstant(3.14), pairs: [ExprPair(EAdd, EConstant(0.9999))] } }, vals:{}, instrs:{} }");
    ok_parse::<f64>("3.14 + 0.", &mut slab);
    assert_eq!(format!("{:?}",&slab),
...
fn aaa_test_b1() {
    ...
    assert_eq!(parse_raw::<f64>("3.14 + 4.99999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999.9999", &mut slab),
Err(Error::ParseFloat("4.99999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999999.9999".to_string())));
    assert_eq!(
        parse_raw::<f64>("3.14 + 4.9999.9999", &mut slab),
        Err(Error::ParseFloat("4.9999.9999".to_string()))
    );
}
...
fn aaa_test_b2() {
    ...
    assert_eq!(
        parse_raw::<f64>("3.14 + .", &mut slab),
        Err(Error::ParseFloat(".".to_string()))
    );
}
...
"Slab{ exprs:{ 0:Expression { first: EConstant(3.0), pairs: [ExprPair(EAdd, EConstant(5.0)), ExprPair(ESub, EStdFunc(EVar(\"xyz\")))] } }, vals:{}, instrs:{} }");
    ok_parse::<f64>("3+5-xyz_abc_def123", &mut slab);
    assert_eq!(format!("{:?}",&slab),
"Slab{ exprs:{ 0:Expression { first: EConstant(3.0), pairs: [ExprPair(EAdd, EConstant(5.0)), ExprPair(ESub, EStdFunc(EVar(\"xyz_abc_def123\")))] } }, vals:{}, instrs:{} }");
    ...
    assert_eq!(format!("{:?}",&slab),
"Slab{ exprs:{ 0:Expression { first: EConstant(3.0), pairs: [ExprPair(EAdd, EConstant(5.0)), ExprPair(ESub, EStdFunc(EVar(\"XYZ_abc_def123\")))] } }, vals:{}, instrs:{} }");
    ok_parse::<f64>("3+5-XYZ_ab*c_def123", &mut slab);
    assert_eq!(format!("{:?}",&slab),
...
fn aaa_test_c1() {
    ...
    assert_eq!(
        parse_raw::<f64>("3+5-XYZ_ab~c_def123", &mut slab),
