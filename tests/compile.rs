...
#[test]
fn slab_overflow() {
    let mut slab: Slab<f64> = Slab::with_capacity(2);
    assert_eq!(
...
fn comp(expr_str: &str) -> (Slab<f64>, Instruction<f64>) {
    let mut slab = Slab::new();
...
fn comp_chk(expr_str: &str, expect_instr: Instruction<f64>, expect_fmt: &str, expect_eval: f64) {
    let mut slab = Slab::new();
    let expr = Parser::new()
        .parse(expr_str, &mut slab.ps)
        .unwrap()
        .from(&slab.ps);
    let instr: Instruction<f64> = expr.compile(&slab.ps, &mut slab.cs);
