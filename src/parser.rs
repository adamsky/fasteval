...
pub struct Expression<T: Num> {
    pub(crate) first: Value<T>,
    pub(crate) pairs: Vec<ExprPair<T>>, // cap=8
}

#[derive(Debug, PartialEq)]
pub(crate) struct ExprPair<T: Num>(pub BinaryOp, pub Value<T>);
...
impl Parser {
    ...
    fn read_expression<T: Num>(
        ...
        let mut pairs = Vec::<ExprPair<T>>::with_capacity(8);
        loop {
    ...
    fn read_const<T: Num>(slab: &mut ParseSlab<T>, bs: &mut &[u8]) -> Result<Token<T>, Error> {
        spaces!(bs);
        ...
        let val = tok
            .parse::<T>()
            .map_err(|_| Error::ParseFloat(tok.to_string()))?;
        skip_n!(bs, toklen);
    ...
    fn read_callable<T: Num>(
        &self,
        slab: &mut ParseSlab<T>,
        bs: &mut &[u8],
        depth: usize,
    ) -> Result<Token<Value<T>>, Error> {
    ...
    fn read_expressionorstring<T: Num>(
        &self,
        slab: &mut ParseSlab<T>,
        bs: &mut &[u8],
        depth: usize,
...
impl<T: Num> Default for Expression<T> {
    fn default() -> Self {
        Expression {
...
impl<T: Num> Default for Value<T> {
    fn default() -> Self {
        EConstant(T::nan())
    }
}
